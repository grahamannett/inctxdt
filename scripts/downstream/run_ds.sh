...
CUDA_VISIBLE_DEVICES=1 python inctxdt/run.py --cmd=downstream --device=cuda --config_path=$CONFIG_PATH --downstream.config_path=$DOWNSTREAM_CONFIG_PATH --num_layers=$NUM_LAYERS --num_heads=$NUM_HEADS --modal_embed.per_action_encode=True --modal_embed.tokenize_action=True --modal_embed.action_embed_class=ActionTokenizedEmbedding --eval_output_sequential=False --batch_size=128 --update_steps=$UPDATE_STEPS --downstream.patch_actions=True --downstream.update_optim_actions=True --downstream.patch_states=True --downstream.update_optim_states=True --downstream.optim_only_patched=True --log.group=$GROUP --log.job_type=UpdateEmb-ActionTokenizedEmbedding --log.mode=online --seed=$SEED --train_seed=$SEED >/dev/null 2>&1 &
...
CUDA_VISIBLE_DEVICES=0 python inctxdt/run.py --cmd=downstream --device=cuda --config_path=$CONFIG_PATH --downstream.config_path=$DOWNSTREAM_CONFIG_PATH --num_layers=$NUM_LAYERS --num_heads=$NUM_HEADS --modal_embed.per_action_encode=True --modal_embed.tokenize_action=True --modal_embed.action_embed_class=ActionTokenizedSpreadEmbedding --eval_output_sequential=False --batch_size=128 --update_steps=$UPDATE_STEPS --downstream.patch_actions=False --downstream.patch_states=True --downstream.optim_use_default=True --log.group=$GROUP --log.job_type=DefaultOptim-ActionTokenizedSpreadEmbedding --log.mode=online --seed=$SEED --train_seed=$SEED >/dev/null 2>&1 &
